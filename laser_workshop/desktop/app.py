# -*- coding: utf-8 -*-
"""
Лазерная Мастерская CRM - Полнофункциональная версия для Windows Desktop
Включает все функции: AI прогнозы, аналитику, интеграции
"""
...
class ConnectionPool:
    ...
    def init_db_schema(self, cursor):
        """Создание полной схемы БД со всеми таблицами"""
        ...
        # Таблица AI прогнозов (включена в Desktop версии)
        cursor.execute('''
...
promo_cache = LRUCache(max_size=100, ttl=60)
...
@token_required
def get_ai_predictions_route(current_user):
    if not AI_PROGNOSIS_ENABLED:
        return jsonify({'message': 'AI прогнозы отключены'})
...
def get_stats(current_user):
    ...
    with db_pool.get_connection() as conn:
        ...
        cursor.execute("SELECT SUM(total_price) as revenue FROM orders WHERE status IN ('DONE', 'DELIVERED')")
