...
class Config:
    ...
    # Основные настройки
    SECRET_KEY = os.getenv('SECRET_KEY', 'laser_workshop_rpi_secret_key_2026_change_this')
    ...
    # Оптимизация производительности (RPi оптимизировано)
    MAX_CONNECTIONS = int(os.getenv('MAX_CONNECTIONS', '5'))
    THREAD_POOL_SIZE = int(os.getenv('THREAD_POOL_SIZE', '2'))
    CACHE_SIZE = int(os.getenv('CACHE_SIZE_MB', '32')) * 1024 * 1024
    
    # Кэширование
    CACHE_TTL = int(os.getenv('CACHE_TTL_SECONDS', '180'))
    MAX_USER_STATES = int(os.getenv('MAX_USER_STATES', '500'))
    ...
    # AI Прогнозирование (отключено для RPi по умолчанию)
    AI_PROGNOSIS = os.getenv('AI_PROGNOSIS_ENABLED', 'False').lower() in ('true', '1', 'yes')
